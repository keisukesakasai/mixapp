...
あわせて **LLM Observability**（Datadog 等）の評価をトリガーするプロンプト（toxicity / sentiment / prompt-injection / failure-to-answer）を混ぜて送信します。
...
| EVAL_PROMPT_RATIO | 0.25 | 評価用プロンプトを送る割合（0.0〜1.0）。0 にすると通常の投資質問のみ |
